...
| Retrieval | Symbol lookup + FTS5 + 1-hop graph | Semantic / hybrid over memories |
...
| `index.rs` | Parallel walk, hash, parse, SQLite write |
| `extract/` | Tree-sitter (TS/JS/Python), markdown headings, CSS selectors |
| `store.rs` | SQLite schema, lookups, FTS, neighbors |
...
| Outline | `.md` `.mdx` `.css` `.scss` | Headings or selectors; no edges |
...
Closed symbol kinds: `module`, `function`, `method`, `class`, `interface`, `type`, `component`, `heading`, `selector`.

Edge kinds: `import` | `call`. Confidence: `high` (import-resolved or same-file unambiguous) | `low` (name-only guess).

## 5. Data store

Path: `<repo>/.engram/index.sqlite` (gitignored). Schema version `1`.

Pragmas: `journal_mode=WAL`, `synchronous=NORMAL`, `foreign_keys=ON`. Query paths open with `query_only=ON`.

```
meta          schema_version, indexed_at, root, counts
...
Indexes: `symbols(name COLLATE NOCASE)`, `symbols(file_id)`, `edges` src/dst, `files(hash)`.

File **bodies are not the source of truth for quotes**. FTS stores content for ranking. The compiler re-reads spans from disk at query time and checks the live Blake3 hash against `files.hash`.
...
query
  ...
  → re-read disk + Blake3; omit stale
...
`why` tags on items: `exact_symbol`, `prefix_symbol`, `fts`, `import_neighbor`, `call_neighbor`, `heading`, `selector`, `path_hint`, and optionally `palace`.
